      ...
      <table v-if="members.length > 0 && !is_fetching" class="w-full whitespace-no-wrap">
        <thead>
          <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
            <th class="px-4 py-3 text-center">Member Name</th>
            <th class="px-4 py-3 text-center">Paid Amount</th>
            <th class="px-4 py-3 text-center">Reference Number</th>
            <th class="px-4 py-3 text-center">Payment Method</th>
            <th class="px-4 py-3 text-center">Date</th>
            <th class="px-4 py-3 text-center">Receipt Status</th>
            <th class="px-4 py-3 text-center">Actions</th>
          </tr>
        </thead>
        <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
          <tr v-for="member in members" class="text-gray-700 dark:text-gray-400">
            <td class="px-4 py-3 text-sm text-center" v-text="member.member"></td>
            <td class="px-4 py-3 text-sm text-center" v-text="`${member.paid_amount.toLocaleString()}/-`"></td>
            <td class="px-4 py-3 text-sm text-center" v-text="member.reference_number"></td>
            <td class="px-4 py-3 text-xs text-center" v-text="member.payment_method"></td>
            <td class="px-4 py-3 text-sm text-center" v-text="member.date"></td>
            <td class="px-6 py-3 text-sm text-center">
              <span v-if="member.receipt_status === 'created'" class="bg-green-100 text-green-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded-sm dark:bg-green-900 dark:text-green-300 p-2 rounded-md">Created</span>
              <span v-else class="bg-red-100 text-red-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded-sm dark:bg-red-900 dark:text-red-300 p-2 rounded-md">In Progress</span>
            </td>
            <td class="px-4 py-3 text-center">
              <div class="flex items-center space-x-4 text-sm">
                ...
                <button @click="getReceipt(member.id)" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray">
                  <i class="fa-solid fa-receipt" style="color: #7e3af2;"></i>
               </button>
               <button @click="mailTo(member.id)">
                  <i class="fa-solid fa-envelope" style="color: #7e3af2;"></i>
               </button>
              </div>
            </td>
          </tr>
  ...
  <script>
    ...
    const app = Vue.createApp({
      ...
      async mounted() {
        ...
        this.getContent(route("api.member.receipts"));
      },
      watch: {
        search(newValue) {
          this.getContent(route("api.member.receipts", { keyword: newValue }));
        },
