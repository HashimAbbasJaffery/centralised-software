...
const app = Vue.createApp({
    data() {
        return {
            member_name: "{{ $receipt->member->member_name }}",
            paid_amount: "{{ $receipt->paid_amount }}",
            reference_number: "{{ $receipt->reference_number }}",
            payment_method: "{{ $receipt->payment_method->id }}",
            date: "{{ $receipt->date }}",
            selectedId: "{{ $receipt->member->id }}",
    ...
    async submit(e) {
      e.preventDefault();
        const response = await axios.put(
            route("api.member.receipt.update", { receipt: this.receiptId }),
            { 
