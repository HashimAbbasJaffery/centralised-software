<x-layout.app>
  ...
  <main id="app" class="h-full pb-16 overflow-y-auto">
    ...
    <div class="container px-6 mx-auto grid">
      ...
      <a href="{{ route('club.create') }}" style="width: 10%; margin-bottom: 20px; text-align: center;" class="px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
        Create
      </a>
      ...
      <table v-if="clubs.length > 0" class="w-full whitespace-no-wrap">
        <thead>
        ...
        <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
          <tr v-for="club in clubs" class="text-gray-700 dark:text-gray-400">
            <td class="px-4 py-3 text-sm" v-html="club.club_name"></td>
            <td class="px-4 py-3 text-xs" v-text="club.country"></td>
            <td class="px-4 py-3 text-xs" v-text="club.city"></td>
            <td class="px-4 py-3">
              <div class="flex items-center space-x-4 text-sm">
                <button @click="editClub(club.id)" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Edit">
                  <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                  </svg>
                </button>
                <button @click="deleteCard(club.id)" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Delete">
                  <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
  ...
  <script>
    ...
    const app = Vue.createApp({
      ...
      async mounted() {
        ...
        this.getContent(route("api.club.index"));
      },
      watch: {
        search(newValue) {
          this.getContent(route("api.club.index", { keyword: newValue }));
        },
      },
      methods: {
        ...
        async deleteCard(id) {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              const response = await axios.delete(route("api.club.delete", { club: id }));
