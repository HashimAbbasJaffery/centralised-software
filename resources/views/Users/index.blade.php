<x-layout.app>
    ...
    <main id="app" class="h-full pb-16 overflow-y-auto">
        ...
        <div class="container px-6 mx-auto grid">
            ...
            <a href="{{ route('user.create') }}" style="width: 10%; margin-bottom: 20px; text-align: center;"
                    ...
                    <tr v-for="user in users" class="text-gray-700 dark:text-gray-400">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center space-x-4 text-sm">
                                <button @click="editMember(user.id)"
                                ...
                                <button @click="deleteMember(user.id)"
                                    class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                    aria-label="Delete">
                                    <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                ...
                <ul class="inline-flex items-center">
                    <li v-for="(link, index) in links">
                        ...
                        <button @click="changePage(link.url)" v-else v-text="link.label"
                            :class="{ 'bg-purple-600 border-purple-600 rounded-md text-white': link.active ===
                                true, 'text-black': link.active != true }"
    ...
    <script>
        ...
        const app = Vue.createApp({
            ...
            methods: {
                ...
                editMember(id) {
                    window.location = route('user.update', {
                        user: id
                    });
                },
                async deleteMember(id) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            const response = await axios.delete(route("api.user.delete", {
                                user: id
                            }));
