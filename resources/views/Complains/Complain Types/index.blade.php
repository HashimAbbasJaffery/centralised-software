<x-layout.app>
  ...
  <main id="app" class="h-full pb-16 overflow-y-auto">
    ...
    <div class="container px-6 mx-auto grid">
      ...
      <table v-if="complain_types.length > 0 && !is_fetching" class="w-full whitespace-no-wrap">
        <thead>
        ...
        <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
          <tr v-for="complain_type in complain_types" class="text-gray-700 dark:text-gray-400">
            <td class="px-4 py-3 text-sm" v-text="complain_type.complain_type"></td>
            <td class="px-4 py-3 text-sm" v-text="complain_type.total_questions"></td>
            <td class="px-4 py-3">
              <div class="flex items-center space-x-4 text-sm">
                <button @click="update(complain_type.id)" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Edit">
                  <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                  </svg>
                </button>
                <button @click="deleteMember(complain_type.id)" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Delete">
                  <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
  ...
  <script>
    ...
    const app = Vue.createApp({
      ...
      async mounted() {
        ...
        this.getContent(route("api.complains.complain-types.index"));
      },
      watch: {
        search(newValue) {
          this.getContent(route("api.complains.complain-types.index", { keyword: newValue }));
        },
      ...
      methods: {
        ...
        async getContent(url) {
          ...
          this.complain_types = response.data.data;
        ...
        async deleteMember(id) {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              const response = await axios.delete(route("api.complains.complain-types.delete", { complainType: id }));
              if(response.data.status === "200") {
                location.reload();
              } 
            }
