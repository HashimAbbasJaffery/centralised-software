<x-layout.app>
  ...
  <main id="app" class="h-full pb-16 overflow-y-auto">
    ...
    <div class="container px-6 mx-auto flex items-center" style="margin-top: 50px;">
      <div>
        <img style="border-radius: 100%; height: 100px; width: 100px;" src="/storage/{{ $member->profile_picture }}"/>
      </div>
    ...
    <div class="container px-6 mx-auto" style="margin-top: 50px; display: flex; column-gap: 40px;">
      ...
      <div class="tabs">
        <div style="margin-bottom: 20px;">
          <button @click="tab = 'complaint information'" :class="{'bg-purple-600 text-white': tab === 'complaint information'}" style="width: 170px; text-align: left;" class="tab-button px-3 py-1 text-sm font-medium leading-5 transition-colors duration-150 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
            Complaint 
        </button>
        </div>
        <div style="margin-bottom: 20px;">
    ...
    <div v-if="tab === 'complaint information'">
        <p><span style="display: inline-block; width: 200px;">Complain Type:</span> <span editable="true">{{ $complain->complainQuestion->complainType->complain_type }}</span></p>
        <p style="margin-top: 10px;"><span style="display: inline-block; width: 200px;">Question:</span> {{ $complain->complainQuestion->question }}</p>
        <p style="margin-top: 10px;"><span style="display: inline-block; width: 200px; text-transform: capitalize;">Complain:</span> {{ $complain->complain }}</p>
        </div>
