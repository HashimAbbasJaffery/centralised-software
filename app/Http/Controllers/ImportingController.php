    ...
    foreach ($csvData as $index => $member) {
        ...
        if($data) {
            ...
            if($membershipCardData) {
                $profile_picture = str_replace("uploads", "profile_pictures", $membershipCardData[0]["profile_picture"]);
            }
        }
        try {
            $dob = trim($member["date_of_birth"]);
            $doa = trim($member["date_of_applying"]);
            if(!$this->isValidDateFormat(trim($member["date_of_birth"]))) {
                $dob = \Carbon\Carbon::createFromFormat('m/d/Y', trim($member['date_of_birth']))->format('Y-m-d');
            }
            if(!$this->isValidDateFormat(trim($member["date_of_applying"]))) {
        ...
        $member = Member::createQuietly([
            ...
            "recovery_phone_number" => $data[0]["phone_1"] ?? "",
            "alternate_ph_number" => $data[0]["phone_2"] ?? "",
            ...
            "profile_picture" => $profile_picture ?? "profile_pictures/default-user.png",
        ...
        if($associatedRecoverySheets) {
            foreach($associatedRecoverySheets as $index => $recoverySheet) {
                try {
                ...
                $recovery = RecoverySheet::createQuietly([
                    ...
                    "paid" => is_numeric(str_replace(",", "", $recoverySheet["paid"])) ? (int)str_replace(",", "", $recoverySheet["paid"]) : null,
